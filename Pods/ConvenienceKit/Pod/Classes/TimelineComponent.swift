...
public class TimelineComponent <T: Equatable, S: TimelineComponentTarget where S.ContentType == T> : Refreshable {
  ...
  */
  public func removeObject(object: T) {
    ...
    //self.currentRange.upperBound = self.currentRange.endIndex - 1
  ...
  */
  ...
  func loadMore() {
    ...
    let additionalRange = Range(uncheckedBounds: (lower: currentRange.lowerBound, upper: currentRange.upperBound + target!.additionalRangeSize))
    
    target!.loadInRange(range: additionalRange) { posts in
      let newPosts = posts
      ...
      self.currentRange = Range(uncheckedBounds: (lower: self.currentRange.lowerBound, upper: self.content.count))
      self.target!.tableView.reloadData()
    }
  }
